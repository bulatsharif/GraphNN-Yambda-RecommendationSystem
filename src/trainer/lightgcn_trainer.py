...
class LightGCNTrainer:
    ...
    def _run_batch(self, batch):
        ...
        # Filter edges: LightGCN learns from the target interaction graph (homogeneous).
        mask = (batch.edge_type == self.target_event_idx)
        
        # If sampling resulted in no target edges in the subgraph,  we still run forward but propagation won't happen
        edge_index_filtered = batch.edge_index[:, mask]
        ...
        # Extract embeddings for source and destination nodes of the supervision edges
    ...
    def train_epoch(self, epoch_idx: int):
        self.model.train()
        total_loss = 0
        pbar = tqdm(self.train_loader, desc=f"Epoch {epoch_idx} [Train L-GCN]", leave=True)
    ...
    def get_all_embeddings(self) -> torch.Tensor:
        ...
        # For inference, the full graph or the subgraph is used provided by inference loader.
